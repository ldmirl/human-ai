...
from multigrid.constants import OBJECT_TO_IDX, TILE_PIXELS
from multigrid.objects import Agent, Wall, GridObject
...
class Grid:
  ...
  def __init__(self, width: int, height: int):
    ...
    self.grid: list[GridObject | None] = [None] * (width * height)

  def __contains__(self, key: Any) -> bool:
    if isinstance(key, GridObject):
      for e in self.grid:
        if e is key: return True
    elif isinstance(key, tuple):
      for e in self.grid:
        if e is None: continue
        if (e.color, e.type) == key: return True
        if key[0] is None and key[1] == e.type: return True
      return False
  ...
  def set(self, i: int, j: int, v: GridObject | None):
  ...
  def get(self, i: int, j: int) -> GridObject | None:
    assert 0 <= i < self.width
    assert 0 <= j < self.height
    assert self.grid is not None
    return self.grid[j * self.width + i]

  def horz_wall(self, x:int, y:int, length:int=None, obj_type:Callable[[], GridObject]=Wall):
    if length is None: length = self.width - x
    for i in range(0, length): self.set(x + i, y, obj_type())

  def vert_wall(self, x:int, y:int, length:int=None, obj_type:Callable[[], GridObject]=Wall):
    if length is None: length = self.height - y
    for j in range(0, length): self.set(x, y + j, obj_type())
  ...
  def slice(self, topX:int, topY:int, width:int, height:int) -> Grid:
    ...
    for j in range(0, height):
      for i in range(0, width):
        x, y = topX + i, topY + j
        v = self.get(x, y) if 0 <= x < self.width and 0 <= y < self.height else Wall()
        grid.set(i, j, v)
    return grid

  @classmethod
  def render_tile(cls, obj: GridObject, agent: Agent | None, highlight:bool=False, tile_size:int=TILE_PIXELS, subdivs:int=3) -> np.ndarray:
    """Render a tile and cache the result"""

    # Hash map lookup key for the cache
    agent_encode = agent.encode() if agent else None
    key: tuple[Any, ...] = (agent_encode, highlight, tile_size)
    ...
    if obj is not None: obj.render(img)

    # Overlay the agent ontop
    if agent is not None: agent.render(img)
  ...
  def render(self, tile_size: int, agents: List[Agent], highlight_mask: np.ndarray | None = None) -> np.ndarray:
    """
    ...
    width_px, height_px = self.width * tile_size, self.height * tile_size
  ...
  def decode(array: np.ndarray) -> tuple[Grid, np.ndarray]:
    ...
    for i in range(width):
      for j in range(height):
        ...
        v = GridObject.decode(type_idx, color_idx, state)
        grid.set(i, j, v)
  ...
  def process_vis(self, agent_pos: tuple[int, int]) -> np.ndarray:
      ...
      for j in reversed(range(0, self.height)):
        for i in range(0, self.width - 1):
          ...
          cell = self.get(i, j)
          if cell and not cell.see_behind(): continue
          mask[i + 1, j] = True
          mask[i + 1, j - 1] = True
          mask[i, j - 1] = True
